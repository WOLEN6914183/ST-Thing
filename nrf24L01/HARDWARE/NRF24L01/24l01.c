...
{  
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12|GPIO_Pin_15; // PA12->CE P->A15CSN
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP ;    //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;          //PA8 IRQ引脚
...
void SPI1_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13 |GPIO_Pin_14| GPIO_Pin_15;   //PA5->SCK ,PA6->MISO,PA7->MOSI
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  //考虑输出50M
...
{
	u8 i;
	u8 buf[5]={0X05,0X05,0X05,0X05,0X05}; 
...
u8 NRF24L01_Write_Buf(u8 reg, u8 *pBuf, u8 len)
{
  ...
  NRF24L01_CSN = 1;       //关闭SPI传输
	return status;          //返回读到的状态值,返回0说明写入指定寄存器成功
}				   
//启动NRF24L01发送一次数据
//txbuf:待发送数据首地址
//返回值:发送完成状况
